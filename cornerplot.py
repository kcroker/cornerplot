...
def CornerPlot(dfs, df_names, corner_params, weights=None, bandwidth_fac=1, thresh=[68,95], downsample=False, prior=None, \
               cuts=None, limits=None, plot_limits=None, labels=None, ticks=None, title=None, Nbins=50, plot_pts=False, plot_kde=True, plot_hist=False, plot_prior_hist=True, plot_prior_kde=False, print_credible=90, colormap=None, save=False, resolution=300, fill_banan=False):
    """
    ...
    :plot_kde:  boolean that determines whether marginalized posterior KDEs are plotted
    :plot_hist: boolean that determines whether histogram is plotted behind marginalized posterior KDEs
    :plot_prior_hist: boolean that determines whether a histogram is plotted for the priors
    :plot_prior_kde: boolean that determines whether a KDE is generated from the prior samples
        ...
        for idx, param in enumerate(corner_params):
            ...
            if (prior is not None) and (df_idx==len(dfs)-1):
                if prior[param] is not None:
                    ...
                    if plot_prior_hist==True:
                        _ = marg_axs[idx].hist(prior_data, density=True, weights=_weights, histtype='step', color='k', bins=Nbins, \
                                               alpha=(0.4 if plot_prior_kde else 1), linestyle=':', lw=1, orientation="vertical", label=(None if plot_prior_kde else 'Prior'))
            ...
            if _plot_limits[param]:
                marg_axs[idx].set_xlim(_plot_limits[param])
            else:
                # If not, *store* the autogenerated limits so that the are
                # subsequently enforced downstream
                _plot_limits[param]=marg_axs[idx].get_xlim()
